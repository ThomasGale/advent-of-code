...
class d02 : public Solution {
  public:
    ...
    int findNumDifferentChars(const std::string& str1,
    ...
    void Calculate(std::istream& input) override {
        ...
        for (const std::string& line : input_strings) {
            ...
            std::for_each(line.begin(), line.end(),
                          [&char_count](char letter) { char_count[letter]++; });
            std::for_each(char_count.begin(), char_count.end(),
                          [&dupls](auto& char_count_pair) {
                              dupls.insert(char_count_pair.second);
                          });
            std::for_each(dupls.begin(), dupls.end(),
                          [&dupl_count](int dupl) { dupl_count[dupl]++; });
        }
